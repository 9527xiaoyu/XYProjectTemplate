...
+(UIView *)showDrawUIviewWithtitle:(NSString *)boby{
    UIView *selfview = [[UIView alloc]initWithFrame:CGRectMake(0, 0, kScreenWidth, kScreenHeight)];
    selfview.backgroundColor = [UIColor blackColor];
    selfview.alpha = 0.1;
    UIView *view = [[UIView alloc]initWithFrame:CGRectMake((kScreenWidth-200)/2.0, (kScreenHeight-150-64)/2.0, 200, 150)];
...
+(void)sleepSeconds:(float)senconds finishBlock:(nonnull void (^)(void))block
{
...
+(void)mapParams:(id)object fromDictionary:(NSDictionary *)extras
{
    unsigned int count;
